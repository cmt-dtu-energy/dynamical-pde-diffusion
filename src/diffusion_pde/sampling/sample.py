...
def edm_sampler(
    ...
    latents = torch.randn(sample_shape, device=device)
    dtype_f = torch.float32     # net runs in fp32
    dtype_t = torch.float64     # keep time grid in fp64 for stability, as in EDM

    obs_a = obs_a.to(device=device, dtype=dtype_f)
    obs_u = obs_u.to(device=device, dtype=dtype_f)
    mask_a = mask_a.to(device=device, dtype=dtype_f)
    mask_u = mask_u.to(device=device, dtype=dtype_f)
    ...
    # Prepare conditioning t (not used by your Unet right now, but threaded through API).
    B = latents.shape[0]

    labels = labels.to(device=device, dtype=dtype_f)
    ...
    for i, (sigma_cur, sigma_next) in enumerate(zip(sigmas[:-1], sigmas[1:])):  # i = 0..N-1
        ...
        # Compute losses
        a_N, u_N = x_N[:, :ch_a, :, :], x_N[:, ch_a:, :, :]
        dudt = dxdt[:, ch_a:, :, :]

        loss_pde, loss_obs_a, loss_obs_u = loss_fn(a_N, u_N, dudt, obs_a, obs_u, mask_a, mask_u, dx)
        ...
        grad_x = torch.autograd.grad(loss_comb, x_cur, retain_graph=debug)[0]
